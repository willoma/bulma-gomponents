...
e.Div().
	...
	b.Content(
		e.P("Sometimes you may want to render an element only if some condition is true. You can use the ", e.Code("e.If(bool, e.ElementBuilder, ...any)"), " function to add an element only if the condition is true and avoid any calculation if the condition is false:"),
	),
	c.Example(
